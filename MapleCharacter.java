...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
 ...
 /*All custom shit declare here*/
    ...
    private int gml;
    //寵物暫時刪除private List<MaplePet> pets;
...
//寵物暫時刪除                        ret.pets.add(mit.getLeft().getPet());
                    }
                }
...
//        cancelEffectFromBuffStat(MapleBuffStatus.MP_BOOST_PERCENT);
    ...
    public void petUpdateStats(MaplePet pet, boolean active) {
        client.getSession().write(PetPacket.updatePet(pet, getInventory(MapleInventoryType.CASH).getItem((short) (byte) pet.getInventoryPosition()), active));
        /*List<New.ModifyInventory> mods = new LinkedList<>();
        Item Pet = getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition());
        mods.add(new New.ModifyInventory(3, Pet));
        mods.add(new New.ModifyInventory(0, Pet));
        client.sendPacket(modifyInventory(false, mods, this, active));*/
    }
